...
- API、迁移命令和依赖日志统一输出 JSON；GORM 与 MySQL 驱动使用连接级 Zap 适配器
- go-redis 的进程级 Logger 只能在可执行入口、客户端启动前设置，禁止在请求或并发构造函数中重复设置
- 依赖日志不得展开 SQL、参数、驱动错误原文；保留错误类型、数据库编号与耗时，未知文本默认不记录
- HTTP 异常恢复不得记录请求头、正文或 panic 原始值；保留请求编号、路由模板和调用栈
