// select_mysql_test.go 使用独立测试库验证候选选择事务和重复请求的真实数据库行为。
package infra
...
func TestSelectionMySQL(t *testing.T) {
	...
	checkSelection(t, NewRepo(db))
}

// seedSelection 在隔离库中补齐 GORM 创建和幂等索引字段。
func seedSelection(db *gorm.DB) error {
	...
	for _, sql := range []string{
		...
		`CREATE TABLE message_variants (id BIGINT PRIMARY KEY, message_id BIGINT,
...
func checkSelection(t *testing.T, repo *Repo) {
	...
	var count int64
	if err := repo.db.WithContext(ctx).Model(&model.Message{}).Where("source_variant_id = ?", 21).Count(&count).Error; err != nil {
		t.Fatal(err)
	}
	if count != 1 {
