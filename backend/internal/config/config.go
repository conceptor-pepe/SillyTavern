...
func Load() Config {
	return Config{
		...
		ContextBudget:  envInt("AI_CHAT_CONTEXT_BUDGET", 24000),
		OutputTokens:   envInt("AI_CHAT_OUTPUT_TOKENS", 2048),
		EmbeddingURL:   os.Getenv("AI_CHAT_EMBEDDING_URL"),
		EmbeddingKey:   os.Getenv("AI_CHAT_EMBEDDING_KEY"),
		EmbeddingModel: os.Getenv("AI_CHAT_EMBEDDING_MODEL"),
...
// envInt 只接受正整数，缺省采用保守预算。
func envInt(key string, fallback int) int {
	value, err := strconv.Atoi(os.Getenv(key))
	if err != nil || value <= 0 { // audit:allow-no-log pure environment fallback; startup owns configuration logging.
		return fallback
	}
	return value
}
