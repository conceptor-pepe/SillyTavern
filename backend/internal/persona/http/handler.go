// handler.go 公开作者私有人设 API，不暴露公共发布入口。
package personahttp

import (
	"ai-chat/backend/internal/persona/app"
...
func (h *Handler) Routes(e *gin.Engine, auth gin.HandlerFunc) {
	e.GET("/api/v1/personas", auth, h.list)
	e.POST("/api/v1/personas", auth, h.save)
	e.GET("/api/v1/personas/:id", auth, h.find)
	e.PUT("/api/v1/personas/:id", auth, h.save)
	e.DELETE("/api/v1/personas/:id", auth, h.remove)
}
...
func fail(c *gin.Context, err error) {
	...
	case errors.Is(err, domain.ErrInvalid):
		status, code = 400, "INVALID_PERSONA"
